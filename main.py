...
class Ui(qtw.QMainWindow):
    def __init__(self):
        ...
        self.SNsetProductID = self.findChild(qtw.QPushButton, 'SNsetProductID') # Find the button
        ...
        self.genLabel.clicked.connect(self.genLabels)
        ...
        self.SNsetProductID.clicked.connect(self.invSetProdID)
        ...
        self.descriptiondb = self.findChild(qtw.QPlainTextEdit, 'description')
        ...
        self.coverImage2 = self.findChild(qtw.QLabel, 'coverImage_2')
        self.BarcodeLabelPreview = self.findChild(qtw.QLabel, 'BarcodeLabelPreview')

        #INSERT HANDLERS FOR RETURNS
        self.longNameTextBox.returnPressed.connect(self.searchTitle)
        self.titleTextBox.returnPressed.connect(self.invSetProdID)
        ...
        self.githubLink.triggered.connect(self.openGithub)
    ...
    def on_startup(self):
        ...
        pixmap = qtg.QPixmap(os.path.join(current_directory, "images","template.jpg"))
        scaled_pixmap = pixmap.scaled(self.BarcodeLabelPreview.size(), qtc.Qt.KeepAspectRatio, qtc.Qt.SmoothTransformation)
        self.BarcodeLabelPreview.setPixmap(scaled_pixmap)
    ...
    def openGithub(self):
        ...
        url = "https://github.com/zach3697/RedboxInventoryManager" 
        
        # Open the URL in the default browser 
        webbrowser.open(url)

    def genLabels(self):
        number, ok = qtw.QInputDialog.getInt(self, "Start Position", "Please enter the starting position of the labels:")

    def invSetProdID(self):
        invProdImageFile = None
        invProductImage = None
        preQ = "SELECT Value FROM ProductCatalog WHERE [Key] = "
        invProdIDValue = self.ProductID.text()
        print("Quary: ", preQ+invProdIDValue)

        try:
            invProductImage = self.get_value(preQ+invProdIDValue,prof_file_path, [])
            print("Result: ", invProductImage)
            for item_text in invProductImage:
                invProdImageFile = item_text.get("image_file")
            coverLocation = cover_directory + invProdImageFile
            print("Image File Location: ", coverLocation)
            pixmap = qtg.QPixmap(coverLocation)
            scaled_pixmap = pixmap.scaled(self.coverImage2.size(), qtc.Qt.KeepAspectRatio, qtc.Qt.SmoothTransformation)
            self.coverImage2.setPixmap(scaled_pixmap)
            self.addToInvButton.setEnabled(True)

        except Exception as e:
            print(f"Error: {e}")
            self.show_message_box("Error when loading title. Does this title exist?")
    ...
    def resetInputs(self):
        ...
        self.coverImage2.clear()
        self.coverImage2.setText("Cover Image")
        self.addToInvButton.setEnabled(False)
    ...
    def on_item_selected(self, item):
        ...
        self.descriptiondb.setPlainText(title_List[index].get("description"))
    ...
    def on_add_new(self, item):
        ...
        tempState = self.productIDdb.text()
        ...
        if tempState != "":
            ...
            self.descriptiondb.setPlainText(product_Template["description"])
    ...
    def add_to_inv(self):
        ...
        if addValue and productIDValue and snValue:
            for x in range (0, addValue):
                ...
                self.statusBar.showMessage("Product ID: " + productIDValue + " was asssigned to Disk SN: " + snValue)
